    ...
    return (
      <View style={styles.row}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {(item.selectedIngredient?.length > 0
            ? item.selectedIngredient
            : dish.ingredient?.filter(Boolean)
          ).length > 0 && (
        ...
        <Text style={styles.price}>
          €{((dish.prix || 0) * item.quantity).toFixed(2)}
        </Text>
      </View>
    );
  };
